...
static windows_attribute_map group_mssfu_attribute_map[] = 
{
...
static int addNisDomainName(Slapi_Mod *smod,const Slapi_Entry *ds_entry)
{
   ...
   slapi_log_error(SLAPI_LOG_PLUGIN, posix_winsync_plugin_name,
    ...
    do {
        ...
        rc = slapi_search_internal_get_entry( childparent, nisDomainAttr, &entry , posix_winsync_get_plugin_identity());
        if ( rc == 0 ) {
            if ( rc == 0 && entry) {
	...
	if ( rc != 0 ){
	    ...
	    slapi_log_error(SLAPI_LOG_PLUGIN, posix_winsync_plugin_name,
	        "addNisDomainName NisDomain %s found in DN:%s\n", vals[0]->bv_val, slapi_sdn_get_dn(childparent));
...
static void
posix_winsync_dirsync_search_params_cb(void *cbdata, const char *agmt_dn,
...
static void
posix_winsync_pre_ad_search_cb(void *cbdata, const char *agmt_dn,
                             char **base, int *scope, char **filter,
                             char ***attrs, LDAPControl ***serverctrls)
{
    slapi_log_error(SLAPI_LOG_PLUGIN, posix_winsync_plugin_name,
...
static void
posix_winsync_pre_ds_search_entry_cb(void *cbdata, const char *agmt_dn,
                                   char **base, int *scope, char **filter,
                                   char ***attrs, LDAPControl ***serverctrls)
{
/*
...
/*    slapi_ch_free_string(&tmpbase); */
    slapi_log_error(SLAPI_LOG_PLUGIN, posix_winsync_plugin_name,
                    "<-- _pre_ds_search_cb -- end\n");
    return;
}

/* called before the total update to get all entries from the DS to sync to AD */
static void
posix_winsync_pre_ds_search_all_cb(void *cbdata, const char *agmt_dn,
                                 char **base, int *scope, char **filter,
                                 char ***attrs, LDAPControl ***serverctrls)
{
    slapi_log_error(SLAPI_LOG_PLUGIN, posix_winsync_plugin_name,
...
static void
posix_winsync_pre_ad_mod_user_cb(void *cbdata, const Slapi_Entry *rawentry,
                               ...
                               Slapi_Mods *smods, int *do_modify)
{
			...
			rc = slapi_entry_next_attr(ds_entry, attr, &attr)) 
	{
                ...
                if(is_present_local) {
                    ...
                    if (!values_equal) {
                        slapi_log_error(SLAPI_LOG_PLUGIN, posix_winsync_plugin_name,
                            ...
                            slapi_sdn_get_dn(slapi_entry_get_sdn_const(ds_entry)), ad_type);
                        
                        slapi_mods_add_mod_values(smods,LDAP_MOD_REPLACE,ad_type,valueset_get_valuearray(vs));
                        *do_modify = 1;
                   }
                } else {
                    slapi_mods_add_mod_values(smods,LDAP_MOD_ADD,
                            ad_type,valueset_get_valuearray(vs));
        ...
        if (0 == slapi_attr_type_cmp(type,"nsAccountLock", SLAPI_TYPE_CMP_SUBTYPE))
            sync_acct_disable(cbdata, rawentry, ds_entry, ACCT_DISABLE_TO_AD, NULL, smods, do_modify);
    }
    if (slapi_is_loglevel_set(SLAPI_LOG_PLUGIN)){ 
        for (mod = slapi_mods_get_first_mod(smods); mod;
            mod = slapi_mods_get_next_mod(smods)) {
...
static void
posix_winsync_pre_ad_mod_group_cb(void *cbdata, const Slapi_Entry *rawentry,
                                Slapi_Entry *ad_entry, Slapi_Entry *ds_entry,
                                Slapi_Mods *smods, int *do_modify)
{
    LDAPMod *mod = NULL;
...
static void
posix_winsync_pre_ds_mod_user_cb(void *cbdata, const Slapi_Entry *rawentry,
                               Slapi_Entry *ad_entry, Slapi_Entry *ds_entry,
                               Slapi_Mods *smods, int *do_modify)
{
    LDAPMod* mod = NULL;
...
/* add objectclass: posixAccount, uidnumber ,gidnumber ,homeDirectory, loginshell */
                ...
                if(is_present_local) {
                    ...
                    if (!values_equal) {
                        slapi_log_error(SLAPI_LOG_PLUGIN, posix_winsync_plugin_name,
                            ...
                            slapi_sdn_get_dn(slapi_entry_get_sdn_const(ds_entry)), local_type);
    ...
    slapi_log_error(SLAPI_LOG_PLUGIN, posix_winsync_plugin_name,
                    "<-- _pre_ds_mod_user_cb -- end\n");

    return;
}

static void
posix_winsync_pre_ds_mod_group_cb(void *cbdata, const Slapi_Entry *rawentry,
                                Slapi_Entry *ad_entry, Slapi_Entry *ds_entry,
                                Slapi_Mods *smods, int *do_modify)
{
    LDAPMod* mod = NULL;
	...
	if (!is_present_local && do_modify_local) {
        ...
        if (slapi_attr_value_find(oc_attr, slapi_value_get_berval(voc)) != 0) {
            ...
            slapi_mods_add_mod_values(smods,LDAP_MOD_REPLACE,"objectClass",valueset_get_valuearray(oc_vs));
...
static void
posix_winsync_pre_ds_add_user_cb(void *cbdata, const Slapi_Entry *rawentry,
                               Slapi_Entry *ad_entry, Slapi_Entry *ds_entry)
{
    Slapi_Attr *attr = NULL;
    char *type = NULL;
...
/* add objectclass: posixAccount, uidnumber, gidnumber, homeDirectory, loginShell */ 
         ...
         slapi_entry_next_attr(ad_entry, attr, &attr))
    {
        ...
        while ( attr_map[i].windows_attribute_name != NULL) {
			if ( slapi_attr_type_cmp(attr_map[i].windows_attribute_name, type, SLAPI_TYPE_CMP_SUBTYPE) == 0 )  {
    ...
    sync_acct_disable(cbdata, rawentry, ds_entry, ACCT_DISABLE_TO_DS,
                      ds_entry, NULL, NULL);
    slapi_log_error(SLAPI_LOG_PLUGIN, posix_winsync_plugin_name,
                    "<-- _pre_ds_add_user_cb -- end\n");

    return;
}

static void
posix_winsync_pre_ds_add_group_cb(void *cbdata, const Slapi_Entry *rawentry,
                                Slapi_Entry *ad_entry, Slapi_Entry *ds_entry)
{
    Slapi_Attr *attr = NULL;
    char *type = NULL;
...
/* mod if changed objectclass: posixAccount, uidnumber, gidnumber, homeDirectory, loginShell */ 
                 ...
                 mod = slapi_mods_get_next_mod(smods)) 
    {
        ...
        while ( attr_map[i].windows_attribute_name != NULL) {
            if (0 == slapi_attr_type_cmp(mod->mod_type,attr_map[i].ldap_attribute_name, SLAPI_TYPE_CMP_SUBTYPE)){
                Slapi_Mod *mysmod = slapi_mod_new();
        ...
        if (0 == slapi_attr_type_cmp(mod->mod_type,"nsRoleDN", SLAPI_TYPE_CMP_SUBTYPE)){
            int dummy=0;
            sync_acct_disable(cbdata, rawentry, (Slapi_Entry *)ds_entry, ACCT_DISABLE_TO_AD, NULL, new_smods,&dummy);
        }
    }
    if (slapi_is_loglevel_set(SLAPI_LOG_PLUGIN)){ 
...
static void
posix_winsync_begin_update_cb(void *cbdata, const Slapi_DN *ds_subtree,
...
static void
posix_winsync_end_update_cb(void *cbdata, const Slapi_DN *ds_subtree,
                          const Slapi_DN *ad_subtree, int is_total)
{
    
    slapi_log_error(SLAPI_LOG_PLUGIN, posix_winsync_plugin_name,"--> posix_winsync_end_update_cb -- begin %d %d\n",
...
static void
posix_winsync_destroy_agmt_cb(void *cbdata, const Slapi_DN *ds_subtree,
...
void posix_winsync_set_plugin_identity(void * identity) 
{
...
*/
int posix_winsync_plugin_init(Slapi_PBlock *pb)
{
    void *plugin_id = NULL;
    ...
    if ( slapi_pblock_set( pb, SLAPI_PLUGIN_VERSION,
         ...
         slapi_pblock_set(pb, SLAPI_PLUGIN_START_FN,
                          (void *) posix_winsync_plugin_start ) != 0 ||
         slapi_pblock_set(pb, SLAPI_PLUGIN_CLOSE_FN,
                          (void *) posix_winsync_plugin_close ) != 0 ||
         slapi_pblock_set( pb, SLAPI_PLUGIN_DESCRIPTION,
                           (void *)&posix_winsync_pdesc ) != 0 )
    {
        slapi_log_error( SLAPI_LOG_FATAL, posix_winsync_plugin_name,
